...
public class AwardViewComponent(IViewModelBinder modelBinder, MvpSelectionsApiClient client)
    : BaseViewComponent(modelBinder, client)
{
    ...
    private async Task AwardTitle(AwardModel model)
    {
        ...
        if (addResponse is { StatusCode: HttpStatusCode.Created, Result: not null })
        {
    ...
    private async Task LoadTitle(AwardModel model)
    {
        ...
        if (titleResponse is { StatusCode: HttpStatusCode.OK, Result: not null })
        {
