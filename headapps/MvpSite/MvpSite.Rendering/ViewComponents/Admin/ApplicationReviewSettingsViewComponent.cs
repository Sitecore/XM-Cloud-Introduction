    ...
    : BaseViewComponent(modelBinder, client)
{
    ...
    // ReSharper disable StringLiteralTypo - Lorem Ipsum
        ...
        while (tasks.Count > 0)
        {
            ...
            if (response is { StatusCode: HttpStatusCode.OK, Result: not null })
            {
                ...
                if (response.Result.Count == 1)
                {
                    ...
                    if (selectionRolesResponse is { StatusCode: HttpStatusCode.OK, Result.Count: >= 1 })
                    {
                        role = selectionRolesResponse.Result.First();
                    }
                    else if (selectionRolesResponse is { StatusCode: HttpStatusCode.OK, Result.Count: 0 })
                    {
                        ...
                        if (createRoleResponse is { StatusCode: HttpStatusCode.Created, Result: not null })
                        {
    ...
    {
        Response<IList<SelectionRole>> selectionRolesResponse = await Client.GetSelectionRolesAsync(applicationId: model.Id);
        if (selectionRolesResponse is { StatusCode: HttpStatusCode.OK, Result: not null })
        {
